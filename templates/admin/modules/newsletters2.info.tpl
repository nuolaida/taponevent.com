...
{if $data}
	...
	<form action="{"/admin.php"|amake_url}" method="post">
		<input type="hidden" name="module" value="{$config_module}" />
		<input type="hidden" name="action" value="sendAddCategoryAct" />
		<input type="hidden" name="form[email_id]" value="{$data.id}" />
		<table class="tbl_form">
			<tbody>
				<tr>
					<td>{"add"|translate}</td>
					...
					<td><input type="submit" value="{"add"|translate}" /></td>
				</tr>
			</tbody>
		</table>
	</form>
	...
	{else}
		...
		<a href="{"/index.php?module=`$config_module`&action=send&id=100"|make_url}" title="{"send"|translate}"><i class="material-icons">email</i></a>
		<br /><br />
			...
			<tbody>
				{foreach $list_sent as $item}
					<tr>
						...
						<td>
							<a href="{"/admin.php?module=`$config_module`&action=usersEdit&id={$item.user_id}"|amake_url}"><i class="material-icons">search</i></a>
							{if !$item.sent_time}
								<a href="{"/admin.php?module=`$config_module`&action=sentDeleteAct&email_id={$data.id}&user_id={$item.user_id}"|amake_url}"><i class="material-icons">delete</i></a>
							{/if}
						</td>
					</tr>
				{/foreach}
	...
	{actions_log table="newsletters2_emails" id=$data.id}
{/if}
