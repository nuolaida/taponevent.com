...
<ul class="main_page_menu">
	<li><a href="{"/admin.php?module=`$config_module`&action=ownersInfo"|amake_url}" title="{"add"|translate}"><i class="material-icons">add</i></a></li>
</ul>
	...
	<tbody>
	    {foreach from=$list item=item}
			<tr>
				<td>
                    {$item.title}
                    {if $item.real_name}<div class="small">{$item.real_name}</div>{/if}
				</td>
				<td>
					<a href="{"/admin.php?module=`$config_module`&action=ownersInfo&id={$item.id}"|amake_url}"><i class="material-icons">edit</i></a>
				</td>
			</tr>
	    {/foreach}
	</tbody>
