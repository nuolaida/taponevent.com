...
<ul class="main_page_menu">
	<li><a href="{"/admin.php?module=newsletters&action=usersAdd"|amake_url}" title="{"add"|translate}"><i class="material-icons">add</i></a></li>
</ul>
	...
	<tbody>
		{foreach from=$list item=item}
			<tr{if $item.unsubscribed} class="inactive"{/if}>
				<td>{$item.email}</td>
				<td>
					<a href="{"/admin.php?module=newsletters&action=usersEdit&id={$item.id}"|amake_url}"><i class="material-icons">edit</i></a>
				</td>
			</tr>
		{/foreach}
	</tbody>
