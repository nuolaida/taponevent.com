...
function mainMap(locationsMap) {
    ...
    var locations = locationsMap.map(function(loc) {
        return [
            locationData(
    ...
    for (var i = 0; i < locationsMap.length; i++) {
        bounds.extend(new google.maps.LatLng({lat: parseFloat(locationsMap[i].coordinates_lat), lng: parseFloat(locationsMap[i].coordinates_lng)}));
    }
